...
def get_data():
    source_url = "https://data.london.gov.uk/download/unemployment-rate-region/8a29ec0c-9de3-4777-832f-49ef8c2b4d14/unemployment-region.xls"
    resp = requests.get(source_url, timeout=60, headers={"User-Agent": "Mozilla/5.0"})
    resp.raise_for_status()
    unemployment = pd.read_excel(io.BytesIO(resp.content), sheet_name=1)
    unemploymentArray = np.array(unemployment)  # getting table
    unemployment_london_real_num = unemploymentArray[
        1, :
    ]  # getting unemployment in thousands in London
    unemployment_london_rate = unemploymentArray[
        17, :
    ]  # getting unemployment rate in London

    unemployment_dates = np.array(
        unemployment.columns.values
    )  # getting dates of recorded unemployment
...
Flow(get_data(), change_path, dump_to_path("data"), printer()).process()
